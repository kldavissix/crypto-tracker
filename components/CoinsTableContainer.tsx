...
const CoinsTable = () => {
  ...
  const { ctxCurrency } = useContext(CurrencyContext) as ICurrencyContext;
  ...
  }, [windowHeight, pageSize, setPageNum, setPageSize]);

  // Reset page number when search/sort conditions change
  // Unless this is the first render, in case of return from detail page

  useEffect(() => {
    if (!firstRender) setPageNum(1);
    ...
    const filteredCoins = allCoins
      ?.filter(
        (coin) =>
          ...
          (!favoritesOnly || (favoritesOnly && favoritesSet.has(coin.symbol)))
      )
      .map((coin) => {
        return {
          ...coin,
  ...
  const fetchCoins = () => axios(CoinList(ctxCurrency));
  const { isLoading, isError, data } = useQuery(
    ["CoinsTable", ctxCurrency],
    fetchCoins
  );
          ...
          {Object.entries(fields).map((field) => {
            ...
            return (
              ...
              >
                ...
                {sortField === field[0] &&
                  (sortDir == "asc" ? (
                    <ChevronUpIcon h={iconSize} w={iconSize} ml="1" />
                  ) : (
                    <ChevronDownIcon h={iconSize} w={iconSize} ml="1" />
                  ))}
              </Th>
            );
          })}
    ...
    return (
      <HStack display="flex" pb={6} pt={2} justifyContent="space-between">
        ...
        {buttonLabels.map((label, i) => {
          ...
          const addDisabledStyles = isClickable ? {} : disabledButtonStyle;
          return (
            <Button
              key={i}
              size="xs"
              _focus={{}}
              border="none"
              bgColor={label === String(pageNum) ? "#EEBC1D" : "trasparent"}
              color={label === String(pageNum) ? "black" : "white"}
              {...addDisabledStyles}
              onClick={(e) => {
    ...
    <Box display="flex" justifyContent="center" mt="1">
      <VStack w="90%" maxW="800px">
        ...
        <motion.div
          ...
          style={{ width: "100%" }}
          key={sortField + sortDir + pageNum + ctxCurrency + pageSize}
        >
