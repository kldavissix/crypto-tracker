...
const CoinsTableRows = ({ coins, handleCoinRowClicked }: ICoinsTableRows) => {
  const { ctxCurrencyIcon } = useContext(CurrencyContext) as ICurrencyContext;
    ...
    <>
      {coins.map((coin) => {
        ...
        if (priceChange24HoursDisplay === "-0.00%")
          priceChange24HoursDisplay = priceChange24HoursDisplay.replace(
            "-",
            ""
          );
        ...
        return (
          <Tr
            onClick={(e) => handleCoinRowClicked(id, symbol, favoriteAction)}
            cursor="pointer"
            key={id}
          ...
          >
            ...
            <Td>
              <HStack spacing="0" justifyContent="flex-end">
                {ctxCurrencyIcon}
                <Text>{numberWithCommas(current_price.toFixed(2))}</Text>
              </HStack>
            </Td>
            ...
            <Td>
              <HStack spacing="0" justifyContent="flex-end">
                {ctxCurrencyIcon}
                <Text>
                  {numberWithCommas(market_cap.toString().slice(0, -6))}
                  &nbsp;M
                </Text>
              </HStack>
            </Td>
